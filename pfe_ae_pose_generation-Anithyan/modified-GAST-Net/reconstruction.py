...
from tools.visualization import render_animation_with_pca
...
def load_json(file_path, num_joints, num_person=1):
    ...
    keypoints = np.zeros((num_person, num_frames, num_joints-1, 2), dtype=np.float32)
    scores = np.zeros((num_person, num_frames, num_joints-1), dtype=np.float32)
...
def reconstruction(args):
    ...
    #input_keypoints = normalize_screen_coordinates(keypoints[..., :2], w=2, h=2)
    ...
    np.save("preds3D",prediction_new)
    print('Rendering ...')
    anim_output = {'Reconstruction': prediction_new}
    print(os.getcwd())
    coded_data = np.load("coded_data.npy")
    pca_xyz = np.load("pca_xyz.npy")
    ...
    render_animation_with_pca(keypoints, keypoints_metadata, anim_output, coded_data, pca_xyz, h36m_skeleton, 25, 3000,
                     np.array(70., dtype=np.float32), args.viz_output, limit=-1, downsample=1, size=5,
                     input_video_path=None, viewport=(width, height), input_video_skip=0)
